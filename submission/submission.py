...
class Submission:
    ...
    def get_model_challenge_1(self):
        ...
        model = Vit_EEG_Encoder(emb_dim=384 ,nhead=6 , target_c_dim=32   ).to(self.device)

        rt_regressor_model = Vit_EEG_RT_Decoder(model , emb_dim=384 , fine_tune_encoder=False).to(self.device)
        rt_regressor_model.load_state_dict(torch.load(resolve_path("weights_challenge_1.pt"), map_location=self.device))
        
        rt_regressor_model.eval()
        return rt_regressor_model
    ...
    def get_model_challenge_2(self):
        ...
        pfactormodel = Challange2_Vit_EEG_RT_Decoder(encoder , decoder).to(self.device)
        
        pfactormodel.load_state_dict(torch.load(resolve_path("weights_challenge_2.pt"), map_location=self.device))
        pfactormodel.eval()
        return pfactormodel
