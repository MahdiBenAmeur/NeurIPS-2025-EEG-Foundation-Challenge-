...
def main():
    ...
    shard_size = 1000
    shards_path= r"D:\projects\pytorch_training\ccd_shards_dir_full"
    ...
    encoder_path = os.path.join(MODELS_AND_CHECKPOINTS_PATH, "demask_enc.pth")

    model.load_state_dict(torch.load(encoder_path)["model"])
    best_model_path = os.path.join(MODELS_AND_CHECKPOINTS_PATH , "challange1_mae_encoder.pt")
    ...
    test_nrmse = nrmse_over_data(rt_regressor_model , test_ccd_dataloader,device)
    print(f"test nRMSE :{test_nrmse}")

    val_nrmse = nrmse_over_data(rt_regressor_model , val_ccd_dataloader , device)
    print(f"val nrmse : {val_nrmse}")

    train_nrmse = nrmse_over_data(rt_regressor_model , train_ccd_dataloader , device)
    ...
    optimizer = torch.optim.AdamW([
        {"params" : rt_regressor_model.encoder.parameters() , "lr" : base_lr*0.5} ,
        {"params": rt_regressor_model.pool.parameters(), "lr": base_lr},
        {"params": rt_regressor_model.regressor.parameters(), "lr": base_lr},
    ...
    best_rnmse_path =os.path.join(MODELS_AND_CHECKPOINTS_PATH , "challange1_mae_encoder_nrmse.pt")
    best_rnmse = 100
    if os.path.exists(best_rnmse_path):
        best_rnmse = float(torch.load(best_rnmse_path))
        print(f"best nrmse loaded with rnmse : {best_rnmse}")
